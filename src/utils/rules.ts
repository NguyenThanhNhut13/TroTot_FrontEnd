...
export const formCreateRoom = yup.object({
  ...
  title: yup.string().required('Tên phòng trọ').max(200, 'Tên phòng trọ không được quá 200 ký tự'),
  ...
  area: yup.number().required('Diện tích là bắt buộc').positive('Diện tích phải là số dương'),
  selfManaged: yup.boolean().required('Quản lý bản thân là bắt buộc'),
  totalRooms: yup.number().required('Tổng số phòng là bắt buộc').positive('Tổng số phòng phải là số dương').integer('Tổng số phòng phải là số nguyên'),
  maxPeople: yup.number().required('Số người tối đa là bắt buộc').positive('Số người tối đa phải là số dương').integer('Số người tối đa phải là số nguyên'),
  forGender: yup.string().oneOf(['ALL', 'MALE', 'FEMALE', 'OTHER'], 'Giới tính không hợp lệ').required('Giới tính là bắt buộc'),
  deposit: yup.number().required('Tiền đặt cọc là bắt buộc').min(0, 'Tiền đặt cọc không được âm'),
  posterName: yup.string().required('Tên người đăng là bắt buộc').max(100, 'Tên người đăng không được quá 100 ký tự'),
  posterPhone: yup.string().required('Số điện thoại người đăng là bắt buộc').matches(/^[0-9]{10,11}$/, 'Số điện thoại không hợp lệ'),
  images: yup.array().of(
    yup.object({
      id: yup.number().required('ID ảnh là bắt buộc'),
      publicId: yup.string().required('Public ID là bắt buộc'),
      imageUrl: yup.string().required('URL ảnh là bắt buộc')
    })
  ).min(0, 'Cần ít nhất 1 ảnh'),
  roomType: yup.string().oneOf(['APARTMENT', 'WHOLE_HOUSE', 'BOARDING_HOUSE'], 'Loại phòng không hợp lệ').required('Loại phòng là bắt buộc'),
  ...
  numberOfLivingRooms: yup.number().min(0, 'Số phòng khách không được âm').integer('Số phòng khách phải là số nguyên'),
  numberOfKitchens: yup.number().min(0, 'Số nhà bếp không được âm').integer('Số nhà bếp phải là số nguyên'),
  numberOfBathrooms: yup.number().min(0, 'Số phòng tắm không được âm').integer('Số phòng tắm phải là số nguyên'),
  numberOfBedrooms: yup.number().min(0, 'Số phòng ngủ không được âm').integer('Số phòng ngủ phải là số nguyên'),
